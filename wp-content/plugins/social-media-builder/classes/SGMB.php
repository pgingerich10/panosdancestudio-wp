...
class SGMB
{
	public function init()
	{
		...
		add_action('the_posts', array($this, 'metashortcode'));
	...
	public function metashortcode($posts) {
		...
		return $this->metashortcode_shortcode_to_wphead($posts, $shortcode, $callback_function);
	}

	// To execute when shortcode is found
	public function metashortcode_setmeta() {
		global $SGMB_META_IMAGE, $SGMB_META_TITLE; $url = $this->full_url($_SERVER);
		if($SGMB_META_IMAGE != '') { ?>
			<meta name="twitter:card" content="summary_large_image">
			<meta name="twitter:title" content="<?php echo $SGMB_META_TITLE; ?>">
			<meta name="twitter:description" content="<?php echo $url; ?>">
			<meta name="twitter:domain" content="<?php echo $url; ?>">
			<meta name="twitter:image" content="<?php echo "$SGMB_META_IMAGE"; ?>" />
			<meta property="og:image" content="<?php echo "$SGMB_META_IMAGE"; ?>"/>
		<?php }
	}
	...
	public function metashortcode_shortcode_to_wphead($posts, $shortcode, $callback_function) {
		...
		global $SGMB_META_IMAGE,$SGMB_META_TITLE;
		foreach ($posts as $post) {
			if (stripos($post->post_content, '['.$shortcode) !== false) {
				add_action('wp_head', array($this, $callback_function));
				$content = $post->post_content;
				$shortcodeParseContent = shortcode_parse_atts($content);
				if(isset($shortcodeParseContent['customimageurl'])) {
					$SGMB_META_IMAGE = $shortcodeParseContent['customimageurl'];
					$SGMB_META_TITLE = $post->post_title;
				}
				break;
			}
		}
	...
	public function showShortCode($args)
	{
		...
		?>
		<?php
		add_action('wp_head', array($this, 'metashortcode_setmeta'));
